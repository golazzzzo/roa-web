    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 flex flex-col gap-4">
        {loading ? (
          ...
          <p className="font-tour text-[9px] tracking-[0.3em] uppercase text-[#2e2e2e] m-auto">Sé el primero en escribir</p>
        ) : (
            ...
            {grouped.map((group) => {
              ...
              return (
                <motion.div
                  ...
                  className={`flex flex-col gap-0.5 max-w-[65%] ${isOwn ? 'self-end items-end' : 'self-start items-start'}`}
                >
                  {/* Sender name + time */}
                  <div className={`flex items-center gap-2 px-1 mb-1 ${isOwn ? 'flex-row-reverse' : ''}`}>
                    <span className="font-display text-[0.85rem] leading-none tracking-[0.08em] text-[#c4bdb0]">
                      {isOwn ? 'TÚ' : name.toUpperCase()}
                    </span>
                    <span className="font-tour text-[8px] tracking-[0.1em] text-[#2e2e2e]">
                      ...
                      <div key={msg.id} className="relative group">
                        <div className={`border overflow-hidden ${
                          ...
                          {msg.media_url && msg.media_type === 'image' && (
                            <img src={msg.media_url} alt="" className="max-w-[280px] max-h-[320px] w-full object-cover block" />
                          )}
                          {msg.media_url && msg.media_type === 'video' && (
                            <video src={msg.media_url} className="max-w-[280px] block" controls />
                          )}
                          {hasText && (
                            <p className="font-tour text-[11px] tracking-[0.04em] text-[#c4bdb0] leading-relaxed px-4 py-2.5">
                              {msg.content}
                            </p>
                          )}
                        </div>
                        {isOwn && (
                          <button
                            ...
                            className="absolute top-1/2 -translate-y-1/2 -left-5 opacity-0 group-hover:opacity-100 transition-opacity font-tour text-[8px] text-[#2e2e2e] hover:text-red-400/50"
                          >✕</button>
                        )}
                      </div>
                    )
