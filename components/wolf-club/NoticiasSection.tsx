...
export default function NoticiasSection() {
  ...
  const publish = async () => {
    ...
    const { data } = await supabase
      .from('news_posts')
      .insert({ title: title.trim(), body: body.trim(), image_url: imageUrl.trim() || null })
      .select()
      .single()
    ...
    <div className="px-8 md:px-16 py-12 max-w-3xl">
      ...
      {isAdmin && (
        <div className="mb-12 border border-[#1f1f1f] p-6 flex flex-col gap-4">
          <p className="font-tour text-[8px] tracking-[0.3em] uppercase text-[#3a3a3a]">Nueva noticia</p>
          <input
            value={title}
